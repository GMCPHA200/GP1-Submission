...
cSprite::cSprite(D3DXVECTOR3 sPosition, LPDIRECT3DDEVICE9 pd3dDevice, cD3DXTexture* inTexture) // Constructor with texture
{
	D3DXVECTOR2 translation = D3DXVECTOR2(0,0);
	cSprite::setSpritePos(sPosition);
	//cSprite::setSpritePos2D(sPosition);	
	cSprite::setTexture(inTexture);
...
cSprite::cSprite(D3DXVECTOR3 sPosition, LPDIRECT3DDEVICE9 pd3dDevice, LPCSTR theFilename) // Constructor that creates new texture
{
	D3DXVECTOR2 translation = D3DXVECTOR2(0,0);
	cSprite::setSpritePos(sPosition);
	//cSprite::setSpritePos2D(sPosition);
	//Added this to stop null pointer errors
	cSprite::spriteTexture = new cD3DXTexture();
